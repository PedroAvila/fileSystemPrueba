...
export class ArchivoService {
    ...
    private async extractZipBuffer(zipBuffer: Buffer, password: string): Promise<Map<string, Buffer>>{
        ...
        return new Promise((resolve, reject) => {
            ...
            const process = spawn(this.sevenZipPath, ['x', tempZipPath, `-p${password}`, `-o${tempExtractPath}`, '-y']);
            ...
            process.on('close', (code) => {
                if (code === 0) {
                    ...
                    reject(new Error(`Error al descomprimir: ${Buffer.concat(errorData).toString()}`));
                }
            });
        });
    }

    private readFilesRecursively(directory: string, extractedFiles: Map<string, Buffer>) {
